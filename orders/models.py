...
#     def __str__(self):
#         return f"Order: {self.order.auto_order_no}, Item: {self.item.name}, Quantity: {self.quantity}"


# class Order(BaseModel):
#     auto_order_no = models.CharField(max_length=20, unique=True, editable=False)
#     order_date = models.DateTimeField(auto_now_add=True)
#     total = models.FloatField(default=0)
#     discount = models.FloatField(default=0)
#     final_total = models.FloatField(editable=False)
#     items = models.ManyToManyField(OrderItem)  # Remove this line

#     def save(self, *args, **kwargs):
#         # Calculate total and final total before saving
#         self.total = sum(
#             order_item.total_price for order_item in self.orderitem_set.all()
#         )
#         self.final_total = self.total - self.discount
#         super().save(*args, **kwargs)

#     def __str__(self):
#         return f"Order No: {self.auto_order_no}, Total: {self.final_total}"
